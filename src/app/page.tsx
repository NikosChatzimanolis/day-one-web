...
const t = {
  ...
  ru: {
    nav: { services: "Услуги", work: "Проекты", contact: "Контакты" },
    hero: {
      title1: "Понятный, современный сайт для вашего бизнеса ",
      titleHighlight: "онлайн",
      title2: ".",
      desc: "Мы разрабатываем простые и быстрые сайты для небольших бизнесов на Кипре и в Греции и подключаем их к инструментам, которыми вы уже пользуетесь.",
      sub: "Небольшая команда, которая берёт на себя дизайн, разработку и обновления вместе с вами, шаг за шагом.",
      cta: "Запросить бесплатный макет",
      note: "Мы зададим несколько вопросов, соберём простой макет и отправим вам предварительный вариант до того, как нужно будет что‑то решать."
    },
    services: {
      sectionTitle: "Что мы можем снять с ваших плеч",
      s1: {
        title: "Разработка сайтов",
        desc: "Небольшие, быстрые сайты с понятными разделами: кто вы, что предлагаете, примеры работ и простой способ связаться с вами."
      },
      s2: {
        title: "Ведение социальных сетей",
        desc: "Планируем ежемесячный контент, пишем тексты и отправляем вам на утверждение перед публикацией."
      },
      s3: {
        title: "Присутствие в Google",
        desc: "Создаём или настраиваем профиль Google Business, добавляем фото, базовое SEO и следим, чтобы вас могли легко найти."
      },
      s4: {
        title: "Фотосъёмка и обработка",
        desc: "Съёмка вашего пространства, команды или товаров с последующей обработкой, чтобы всё выглядело чисто и единообразно на сайте и в соцсетях."
      },
      s5: {
        title: "Бренд‑стратегия",
        desc: "Помогаем определить тон, цвета и простые правила, чтобы сайт, посты и сообщения воспринимались как единый бренд."
      },
      s6: {
        title: "Дизайн вокруг вашего рабочего дня",
        desc: "Мы начинаем с того, как вы уже работаете — звонки, WhatsApp, email — и подстраиваем под это ваше онлайн‑присутствие, а не наоборот."
      }
    },
    work: {
      sectionTitle: "Текущие проекты",
      note: "(публикуются с разрешения клиента)",
      p1: {
        tag: "Сайт",
        title: "Opostofantastikes.gr",
        desc: "Сайт в процессе разработки для event‑планера с фокусом на свадьбах. Цель — простой одностраничный сайт с разделом «обо мне», понятным списком услуг и удобным способом связаться, чтобы потенциальные клиенты быстро понимали её стиль и могли отправить заявку."
      },
      p2: {
        tag: "Сайт и брендинг",
        title: "Idees kai Luseis",
        desc: "Разработка сайта и лёгкий ребрендинг для компании по ремонту в Греции. Мы структурируем услуги в понятные пакеты, планируем галерею с фотографиями «до и после» и делаем так, чтобы сайт и социальные сети выглядели единообразно по мере роста бизнеса."
      }
    },
    process: {
      sectionTitle: "Как мы работаем",
      p1: { title: "Глубоко понимаем ваше видение", desc: "Мы уделяем время тому, чтобы понять ваши цели, аудиторию и позиционирование, создавая надёжную основу до начала работы." },
      p2: { title: "Разрабатываем индивидуальную стратегию", desc: "Мы создаём структурированный план и цифровое присутствие, адаптированное именно под ваш бизнес и его задачи." },
      p3: { title: "Запускаем и развиваем вместе с вами", desc: "Воплощаем стратегию в жизнь, помогаем запустить её уверенно и остаёмся рядом для постоянной поддержки и оптимизации." }
    },
    pricing: {
      sectionTitle: "Гибкое ценообразование",
      p1: { title: "Проекты по созданию сайта", desc: "От " },
      p2: { title: "Соцсети и брендинг", desc: "Стоимость по результатам созвона" },
      p3: { title: "Сопровождение и абонементы", desc: "Доступно по запросу" },
      note: "Большинство одностраничных сайтов с 4–6 разделами, адаптивным дизайном и простой формой обратной связи начинаются от 350 € и зависят от объёма контента и дополнительных функций."
    },
    about: {
      badge: "Помогаем превращать идеи в реальные цифровые продукты.",
      title: "О студии Day One",
      p1: "Day One — небольшая студия, которая разрабатывает сайты для малых бизнесов на Кипре и в Греции.",
      p2: "Часто проекты начинаются с чего‑то незавершённого: недоделанный сайт на Wix или WordPress, логотип в папке и несколько скриншотов примеров. Этого достаточно — мы помогаем превратить это в понятный, цельный сайт и простой план по вашему онлайн‑присутствию.",
      p3: "Мы предпочитаем короткие созвоны, наглядные примеры и ранние черновики вместо длинных документов. Вы быстро видите работу в процессе, даёте обратную связь, и мы корректируем всё вместе.",
      p4: "Мы не стремимся стать большим агентством. Берём ограниченное количество проектов, остаёмся на связи по WhatsApp и email и создаём сайты, которыми вам захочется делиться и которые вы сможете поддерживать в актуальном состоянии."
    },
    contact: {
      title: "Давайте обсудим ваш проект",
      desc: "Расскажите, что вам нужно, и мы подготовим бесплатный макет, чтобы вы увидели, как может выглядеть ваш сайт, ещё до начала работы.",
      l1: "Бесплатный макет",
      l2: "Без обязательств",
      l3: "Быстрый ответ",
      call: "Позвонить нам",
      ...
      formEmail: "Email",
      formMsg: "Сообщение",
      formCTA: "Запросить бесплатный макет",
      formSending: "Отправка...",
      formSuccess: "Сообщение отправлено!",
      formError: "Ошибка отправки. Попробовать ещё раз?",
      formNote: "Обычно мы отвечаем в течение 24 часов.",
      waTitle: "Предпочитаете быстрые сообщения?",
      waCTA: "Написать в WhatsApp"
    },
    footer: {
      p1: "Всё, что нужно вашему бизнесу онлайн.",
      p2: "Сейчас работаем с клиентами на Кипре и в Греции."
    }
  }
};
        ...
        <div className="container mx-auto px-4 md:px-8 lg:px-12 flex items-center justify-between max-w-[1200px]">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 text-[14px] font-inter font-semibold sm:border-l sm:border-gray-300 sm:pl-6">
              ...
              <button 
                onClick={() => setLang('ru')} 
                className={`transition-colors duration-200 ${lang === 'ru' ? 'text-[#3b82f6]' : 'text-gray-400 hover:text-gray-600'}`}
              >
                RU
              </button>
            </div>
